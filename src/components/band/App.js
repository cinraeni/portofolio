...
const PP_TEXTURE_PATH = '/assets/pp.png';

useGLTF.preload(GLTF_PATH);
useTexture.preload(TEXTURE_PATH);
useTexture.preload(PP_TEXTURE_PATH);
...
function Band({ isMobile, maxSpeed = 50, minSpeed = 10 }) {
  ...
  const ppTexture = useTexture(PP_TEXTURE_PATH);
  ppTexture.flipY = false;
  
  // Anti-stretch & Zoom (object-fit: cover behavior)
  if (ppTexture.image) {
    // Zoom in (0.7) and apply ratio correction for 1080x1920 on a 1.6x2.25 card
    ppTexture.repeat.set(0.7, 0.7 * (0.5625 / 0.711)); 
    ppTexture.offset.set(0.3, 0.15); // Geser X ke kiri 30%, Y ke bawah 15%
  }
        ...
        >
          ...
          >
            <mesh geometry={nodes.card.geometry}>
              <meshPhysicalMaterial {...materials.base} map={ppTexture} />
            </mesh>
